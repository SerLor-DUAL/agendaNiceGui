...
class User(SQLModel, table=True):
    ...
    # UUID column - stores a unique identifier per user
    uuid: Optional[UUID] = Field(
                                default=None,
                                sa_column=Column(ut.USERS_UUID_COL, String, unique=True, nullable=True)
                            )
    
    # ID column - stores a unique integer identifier for each user
